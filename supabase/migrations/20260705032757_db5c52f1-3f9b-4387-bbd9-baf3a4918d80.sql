-- Modelo: taxa_entrega (cliente) = frete global + taxa por pedido da loja.
-- Entregador recebe apenas o frete global; a taxa por pedido fica retida
-- com a loja e é repassada ao sistema via cobrança.

-- 1) Ganho do dia do entregador desconta a taxa por pedido aplicada
...
-- 2) Crédito ao entregador na entrega usa apenas a parcela do frete global
...
-- 3) Ao atribuir manualmente um entregador, recalcular taxa_entrega
--    somando a taxa por pedido snapshot (frete + plano)
...
-- 4) aceitar_pedido_externo: taxa_entrega = frete global + taxa por pedido aplicada
