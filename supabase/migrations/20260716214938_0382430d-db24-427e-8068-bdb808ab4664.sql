...
BEGIN
  ...
  -- Regra: entregador só pode ter 1 pedido ativo por vez.
  -- Exceção: quando a LOJA já agrupou vários pedidos na mesma rota
  -- (rota_id definido antes do aceite), o entregador pode aceitar
  -- todos os pedidos daquele mesmo rota_id.
  ...
  IF _ativos > 0 THEN
    IF _p.rota_id IS NULL THEN
      RAISE EXCEPTION 'Você já tem um pedido ativo. Finalize a entrega atual antes de aceitar outro.';
    END IF;

    SELECT COUNT(*) INTO _ativos_mesma_rota
      FROM public.pedidos
     WHERE entregador_id = auth.uid()
       AND rota_id = _p.rota_id
       AND status IN ('em_rota'::pedido_status, 'coletado'::pedido_status);

    IF _ativos_mesma_rota = 0 THEN
      RAISE EXCEPTION 'Você já tem um pedido ativo de outra loja/rota. Finalize antes de aceitar este.';
    END IF;
