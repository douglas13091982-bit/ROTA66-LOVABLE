...
export function PedidoCard({ pedido: p, destaque, agrupado }: Props) {
  ...
  const [revealedEntregaLocal, setRevealedEntrega] = useState(false);
  // A chegada é persistida no banco (chegou_entrega_at) — assim o estado
  // sobrevive a recarregar o app e o rastreio do cliente fica em sincronia.
  const revealedEntrega = revealedEntregaLocal || !!p.chegou_entrega_at;
  ...
  async function onChange(v: string) {
    ...
    if (v.length === 4) {
      ...
      if (p.retorno_maquina && p.endereco_coleta) {
        ...
        {isColeta ? (
          ...
          >
            ...
            <div className="flex justify-center">
              ...
              >
                <InputOTPGroup>
                  <InputOTPSlot index={0} className="h-14 w-12 text-2xl text-[#0f1b2d] border-[#0f1b2d]/20" />
                  <InputOTPSlot index={1} className="h-14 w-12 text-2xl text-[#0f1b2d] border-[#0f1b2d]/20" />
                  <InputOTPSlot index={2} className="h-14 w-12 text-2xl text-[#0f1b2d] border-[#0f1b2d]/20" />
                  <InputOTPSlot index={3} className="h-14 w-12 text-2xl text-[#0f1b2d] border-[#0f1b2d]/20" />
                </InputOTPGroup>
              </InputOTP>
            </div>
            {loading && (
