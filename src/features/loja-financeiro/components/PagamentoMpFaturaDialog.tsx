...
import {
  gerarPagamentoAgregadoCobrancas,
  consultarStatusFatura,
} from "@/lib/cobrancas-mp.functions";
...
type Resultado =
  ...
  | { fatura_id: string; metodo: "cartao"; init_point: string; valor: number };

export function PagamentoMpFaturaDialog({
  open,
  onClose,
  cobrancaIds,
  valorTotal,
  ...
  const gerar = useServerFn(gerarPagamentoAgregadoCobrancas);
  const consultar = useServerFn(consultarStatusFatura);
  ...
  const status = useQuery({
    queryKey: ["fatura-mp-status", resultado?.fatura_id],
    enabled: !!resultado?.fatura_id,
    refetchInterval: 5000,
    queryFn: () => consultar({ data: { fatura_id: resultado!.fatura_id } }),
  });
  useEffect(() => {
    if (status.data?.pago) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-card border border-border rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h3 className="font-display text-lg">Pagar todas as taxas via MP</h3>
        ...
        <div className="p-4 space-y-4">
          <p className="text-sm text-muted-foreground">
            {cobrancaIds.length} cobrança(s) — total{" "}
            <strong className="text-foreground">R$ {valorTotal.toFixed(2)}</strong>
          </p>
          ...
          {status.data?.pago ? (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-xs text-muted-foreground">Todas as cobranças foram quitadas.</p>
              <button
                onClick={onClose}
            ...
            <>
              ...
              <button
                ...
                disabled={!email || mGerar.isPending || cobrancaIds.length === 0}
