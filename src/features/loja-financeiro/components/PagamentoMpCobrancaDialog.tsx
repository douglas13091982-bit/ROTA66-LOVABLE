...
export function PagamentoMpCobrancaDialog({
  open,
  onClose,
  cobrancaId,
  defaultEmail,
  ...
  const gerar = useServerFn(gerarPagamentoCobranca);
  const consultar = useServerFn(consultarStatusCobranca);
  ...
  const status = useQuery({
    queryKey: ["cob-mp-status", cobrancaId, resultado?.metodo],
    enabled: !!cobrancaId && !!resultado,
    refetchInterval: 5000,
    queryFn: () => consultar({ data: { cobranca_id: cobrancaId! } }),
  });
  useEffect(() => {
    if (status.data?.pago) {
  ...
  if (!open || !cobrancaId) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-card border border-border rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h3 className="font-display text-lg">Pagar taxas por pedido</h3>
